...
FileProgress.prototype.setComplete = function (up, info) {
    ...
    if (res.url) {
        url = res.url;
        str = "<div><strong>Link:</strong><a id='download-url' href=" + res.url + " target='_blank' > " + res.url + "</a></div>" +
        "<div class=hash><strong>Hash:</strong>" + res.hash + "</div>";
    } else {
        var domain = up.getOption('domain');
        url = domain + encodeURI(res.key);
        var link = domain + res.key;
        str = "<div><strong>Link:</strong><a id='download-url' href=" + url + " target='_blank' > " + link + "</a></div>" +
        "<div class=hash><strong>Hash:</strong>" + res.hash + "</div>";
    }
    ...
    if (!isImg) {
        showImg.attr('src', 'http://niu8.qiniudn.com/default.png').css("width","50px").css("height","50px");
